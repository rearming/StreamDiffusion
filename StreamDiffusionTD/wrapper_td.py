...
class StreamDiffusionWrapper:
    ...
    def _load_model(
        ...
        # Debug logging to file (survives segfaults if flushed)
        _dbg_log_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "load_debug.log")
        def _dbg(msg):
            with open(_dbg_log_path, "a") as _f:
                import datetime
                _f.write(f"[{datetime.datetime.now()}] {msg}\n")
            print(msg, flush=True)
        _dbg(f"Starting model load: {model_id_or_path}, pipeline: {pipeline_class.__name__}, device: {self.device}, dtype: {self.dtype}")
        ...
        try:
            ...
            try:
                _dbg(f"from_pretrained({model_id_or_path}, {kwargs})...")
                pipe = pipeline_class.from_pretrained(
                    model_id_or_path,
                    **kwargs
                )
                _dbg(f"from_pretrained OK. Moving to {self.device}...")
            ...
            # except huggingface_hub.utils._errors.LocalEntryNotFoundError:
                ...
                _dbg(f"First load attempt failed: {type(e).__name__}: {e}")
                # If local files are not found, try again without local_files_only
                kwargs.pop('local_files_only', None)
                _dbg(f"Retrying from_pretrained without local_files_only...")
                pipe = pipeline_class.from_pretrained(
                    model_id_or_path,
                    **kwargs
                )
                _dbg(f"from_pretrained OK (retry). Moving to {self.device}...")
                pipe = pipe.to(device=self.device, dtype=self.dtype)
                _dbg("pipe.to() OK (retry)")
        ...
        try:
            if acceleration == "tensorrt": 
                ...
                _dbg_log = os.path.join(engine_dir, "trt_debug.log")
                def _dbg(msg):
                    with open(_dbg_log, "a") as _f:
                        _f.write(f"{msg}\n")
                    print(msg, flush=True)

                _dbg(f"unet_path: {unet_path}, exists: {os.path.exists(unet_path)}")
                _dbg(f"vae_decoder_path: {vae_decoder_path}, exists: {os.path.exists(vae_decoder_path)}")
                _dbg(f"vae_encoder_path: {vae_encoder_path}, exists: {os.path.exists(vae_encoder_path)}")

                if not os.path.exists(unet_path):
                    os.makedirs(os.path.dirname(unet_path), exist_ok=True)
                    _dbg("Creating UNet TRT model object...")
                    unet_model = UNet(
                        fp16=True,
                    ...
                    _dbg(f"UNet model created. batch_size={stream.trt_unet_batch_size}, embedding_dim={stream.text_encoder.config.hidden_size}, unet_dim={stream.unet.config.in_channels}")
                    _dbg("Compiling TensorRT UNet... This may take a moment...")
                    time.sleep(1)
                    compile_unet(
                        stream.unet,
                        unet_model,
            ...
            with open(os.path.join(engine_dir, "acceleration_error.log"), "w") as f:
